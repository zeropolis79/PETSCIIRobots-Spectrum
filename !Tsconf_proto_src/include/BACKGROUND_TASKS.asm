HOVERBOT_SPEED		equ 8 ; PET value 10
HOVERBOT_ANIM_SPEED	equ 2 ; PET value 3
...
RAFT_RIGHT:
	...
	; JMP	WARF5B
	ld hl,UNIT_LOC_X	;raft
	call unit_abs_x
	ld a,(hl)
	ld hl,UNIT_LOC_X	;player
	cp (hl)
	jr nz,WARF05
	ld hl,UNIT_LOC_Y	;raft
	call unit_abs_x
	ld a,(hl)
	ld hl,UNIT_LOC_Y	;player
	cp (hl)
	jr nz,WARF05
	ld hl,UNIT_LOC_X
	inc (hl)			;player
...
RFL2:
	...
	ld hl,UNIT_TYPE
	call offc_abs_x
	ld (hl),14			;pistol fire left AI
	ld a,245			;tile for horizontal weapons fire
...
RFU2:
	...
	ld hl,UNIT_TYPE
	call offc_abs_x
	ld (hl),12		;pistol fire UP AI
	ld a,244		;tile for horizontal weapons fire
	jp ROLLERBOT_AFTER_FIRE
...
ROLAN2:
	...
	ld hl,UNIT_TIMER_B
	call unit_abs_x
	ld (hl),3			;RESET ANIMATE TIMER
	ld hl,UNIT_TILE
	call unit_abs_x
	ld a,(hl)
	cp 164
	jr nz,ROLAN1
	ld (hl),165			;ROLLERBOT TILE
...
TRPL1:
	...
	ld hl,UNIT_TYPE
	call unit_abs_x
	ld (hl),23			;Convert to different AI
	ld hl,UNIT_TIMER_A
	call unit_abs_x
	ld (hl),5
	ld hl,UNIT_TIMER_B
...
REST5:
	...
	ld hl,UNIT_TIMER_A
	call offc_abs_x
	ld (hl),10			;How long until exposion?
	ld hl,UNIT_A
	call offc_abs_x
	ld (hl),0
	ret
...
TC_OPEN_STATE:
	...
	ld (MAP_Y),a
	call GET_TILE_FROM_MAP
	cp 148	;Usual tile for trash compactor danger zone
	jr nz,TRS15
...
TRS10:
	...
	cp 255
	jr nz,TRS15
	jp AILP			;Nothing found, do nothing.
...
WD02:
	...
	ld hl,UNIT_TYPE
	call unit_abs_x
	ld (hl),8			;Dead robot type
	ld hl,UNIT_TIMER_A
	call unit_abs_x
	ld (hl),255
	ld hl,UNIT_TILE
	call unit_abs_x
	ld (hl),115			;dead robot tile
	jp AILP_CHECK_FOR_WINDOW_REDRAW
...
PLASMA_AI_COMMON:
	...
	ld (MAP_Y),a
	call GET_TILE_FROM_MAP
	cp ID_CANNISTER			;cannister tile
	jr z,PLAI11
	ld hl,TILE_ATTRIB
	add a,l
	ld l,a
...
; IND1:
	...
	or a
	ld a,SND_ROBOT_HIT
	call nz,PLAY_SOUND	;only play this sound for non-player units
...
; UD0A:
	...
	ld (hl),0
	ld a,(UNIT_FIND)
	or a				;Is it the player that is dead?
	jr z,UD01
	ld hl,UNIT_TYPE
	call offc_abs_x
	ld b,8				;Dead robot type
	ld a,(hl)
...
HOAT20:
	...
	ld a, SND_ELECTRIC	;electric shock
	call PLAY_SOUND
	ld hl,UNIT_TIMER_A
	call unit_abs_x
...
TE01:
	...
	jr nz,TE01
	;sh8bit -- seems kinda like a bug, explosion shouldn't be added when there is no free slot?
...
TE02:
	...
	ld hl,UNIT_TYPE
	call offc_abs_x
	ld (hl),11			;Small explosion AI type
	ld hl,UNIT_TILE
	call offc_abs_x
	ld (hl),248			;first tile for explosion
	ld hl,UNIT_TIMER_A
...
EVIL20:
	...
	call INFLICT_DAMAGE
	call CREATE_PLAYER_EXPLOSION
	ld a, SND_ELECTRIC	;electric shock sound
	call PLAY_SOUND
	ld hl,UNIT_TIMER_A
	call unit_abs_x
...
RAR2:
	...
	; STA	PROX_DETECT
	; RTS	
	cp 2		; 1 HORIZONTAL TILE FROM PLAYER
			; !!!!!!!!!!!!!!!!!!!
	jr c,RAR3
...
RAR5:
	...
	; STA	PROX_DETECT
	; RTS		
	cp 2		; 1 VERTICAL TILE FROM PLAYER
			; !!!!!!!!!!!!!!!
	jr c,RAR6
	xor a		;player not detected
	ld (PROX_DETECT),a
...
LEFT_RIGHT_DROID:
	...
	ld hl,UNIT_TIMER_A
	call unit_abs_x
	ld (hl), HOVERBOT_SPEED			;reset timer to 10
	ld hl,UNIT_A
	call unit_abs_x	
	ld a,(hl)			;GET DIRECTION
	or a				;0=LEFT 1=RIGHT
